"""PumpPortal live feed: the TokenFeed port.

pump.fun's public socket broadcasts token *creations* only -- a create frame carries
no trade history and no description or image. Those live in the off-chain metadata
JSON at `uri`, fetched here, and the traction metrics come from one of two real paths:
live per-token trade streams when a funded PumpPortal key is configured, otherwise one
Solana Tracker poll per candidate. The cheap half of the gate runs first in both, so
most launches are rejected before any paid call is made.
"""
...
from ...domain.token import Token, curve_pct_from_reserves
from ..market.solana_tracker import SolanaTrackerProvider
...
class PumpPortalFeed:
    ...
    def __init__(self, settings: Settings, data: SolanaTrackerProvider | None = None) -> None:
        self.settings = settings
        self.f = settings.filter
        self.data = data
        self.use_ws_trades = bool(settings.credentials.pumpportal)
        self._queue: asyncio.Queue[Token] = asyncio.Queue(maxsize=QUEUE_MAXSIZE)
        self._pending: dict[str, _Candidate] = {}
        self._seen: OrderedDict[str, None] = OrderedDict()
        # websockets moved this type between releases; the adapter only ever calls
        # send/recv/close on it, so it is held loosely rather than pinned to a version.
    ...
    def _passes(self, token: Token) -> tuple[bool, str]:
        ...
        if token.bonding_curve_pct > self.f.max_curve_pct:
            return False, (
                f"curve_too_far ({token.bonding_curve_pct:.1f}% > {self.f.max_curve_pct}%)"
            )
        return True, "ok"
    ...
    async def _ws_loop(self) -> None:
        ...
        while True:
            try:
                ...
                logger.warning(f"websocket dropped ({type(exc).__name__}: {exc}); "
    ...
    def _ws_url(self) -> str:
        ...
        url = self.settings.endpoints.pumpportal_ws
        key = self.settings.credentials.pumpportal
        if not key:
            return url
    ...
    async def _on_create(self, data: dict) -> None:
        ...
        token = Token.from_create_frame(data)
    ...
    def _on_trade(self, data: dict) -> None:
        ...
        # A malformed amount on one frame must not drop the whole trade update.
    ...
    async def _fetch_metadata(self, mint: str, uri: str) -> None:
        ...
        async with self._metadata_sem:
            ...
            try:
                ...
                logger.warning(f"metadata fetch failed for {mint[:8]}: {type(exc).__name__}")
    ...
    async def _sweep_loop(self) -> None:
        ...
        while True:
            ...
            try:
                ...
                logger.warning(f"sweep error: {type(exc).__name__}: {exc}")
    ...
    async def _poll_metrics(self, candidate: _Candidate) -> None:
        ...
        SolanaTrackerProvider, so the pipeline's own enrichment call moments later is free.
        """
        token = candidate.token
        if self.data is None:
            logger.warning("no data fetcher and no PumpPortal key: cannot read trade metrics")
        ...
        passed, _reason = self._passes(token)
        if passed:
            self._pending.pop(token.address, None)
            self._remember(token.address)
            self.stats["passed"] += 1
            logger.info(f"PASS {token.symbol or '?'} {token.address[:8]} "
                f"buyers={token.unique_buyers} vol={token.volume_sol:.2f} SOL "
