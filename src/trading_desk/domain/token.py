"""The launch itself: pump.fun bonding-curve maths and the parsed create frame."""
...
# pump.fun bonding-curve constants. A fresh curve holds 1_073_000_191 virtual tokens and
# completes when 793_100_000 of them have been bought out, leaving 206_900_000 in reserve.
# Progress is therefore how far vTokensInBondingCurve has walked that span.
...
class Token:
    ...
    has_metadata: bool
    creator_address: str = ""
    # Wall clock of the first sighting, used to age the token.
    first_seen_ts: float = field(default_factory=time.time)

    @classmethod
    def from_create_frame(cls, data: dict) -> Token:
        """Parse a pumpportal `txType: create` message into a Token.

        The frame carries only on-chain facts (mint, name, symbol, curve reserves,
        creator). Description, image and socials live in the off-chain metadata JSON at
        `uri`; the feed adapter fetches those and fills them in afterwards, so they start
        empty and `has_metadata` starts False.
        """
        return cls(
            address=coerce_str(data.get("mint")),
            name=coerce_str(data.get("name")),
            symbol=coerce_str(data.get("symbol")),
            description="",
            image_url="",
            twitter="",
            website="",
            telegram="",
            bonding_curve_pct=curve_pct_from_reserves(
                coerce_float(data.get("vTokensInBondingCurve"))
            ),
            unique_buyers=0,
            volume_sol=coerce_float(data.get("solAmount")),
            age_minutes=0.0,
            has_metadata=False,
            creator_address=coerce_str(data.get("traderPublicKey")),
        )
